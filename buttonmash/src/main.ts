...
class ButtonMash {
  ...
  */
    ...
    wordAdd(inputChar: string): void {
        ...
        if (
            typedLetters < this.wordDictionary[activeWordIndex].length &&
            typedLetters < this.wordDictionary[activeWordIndex].length + 10
        ) {
            ...
            typedLetters > this.wordDictionary[activeWordIndex].length - 1 &&
            typedLetters < this.wordDictionary[activeWordIndex].length + 10
        ) {
    ...
    wordDelete(alt = false): void {
        ...
        if (alt) {
            if (
                typedLetters == 0 &&
                activeWordIndex != 0 &&
                prevWordElement.classList[prevWordElement.classList.length - 1] == "error"
            ) {
                //ctrl-backspace if at start of a word and deletes to the start of the previous word
        ...
        } else if (typedLetters == 0 && activeWordIndex != 0) {
            // Checks if caret(not the poisition itself) is at the start of the word and that the active word isnt the first word
            if (prevWordElement.classList[prevWordElement.classList.length - 1] == "error") {
                // Checks if previous word has an error class
                this.markWordError(prevWordElement);
                this.markWordActive(prevWordElement);
            ...
            if (typedLetters - 1 < this.wordDictionary[activeWordIndex].length) {
                // If within wanted word
                if (typedLetters == 1) {
                    // If on the firstletter
                ...
                // Not within wanted word, therefore can just remove last child
    ...
    caretLetter(letter: Element | null): void {
        ...
        caretStyle.top =
            (
                activeLetterBound.top -
                relativeBounds.top -
                (document.getElementById("caret")!.getBoundingClientRect().height - activeLetterBound.height) / 2
            ).toString() + "px";
    }
    ...
    caretWord(word: Element | null): void {
        ...
        caretStyle.top =
            (
                activeWordBound.top -
                relativeBounds.top -
                (document.getElementById("caret")!.getBoundingClientRect().height - activeWordBound.height) / 2
            ).toString() + "px";
        caretStyle.animationName = "none";
    }
    ...
    markWordError(wordElem: Element | null) {
        ...
        if (wordElem?.className == "word error") {
            ...
            for (let i = 0; i < letterCollection.length; i++) {
                if (
                    letterCollection.item(i)?.className == "incorrect" ||
                    letterCollection.item(i)?.className == "" ||
                    letterCollection.item(i)?.className == "incorrect extra"
                ) {
...
document
    ...
    ?.addEventListener("click", function (e) {
        if ((<HTMLInputElement>e.target).className != "buttons") {
            ...
            switch (
                (<HTMLInputElement>e.target).getAttribute("id") ||
                (<HTMLInputElement>e.target).parentElement?.getAttribute("id")
            ) {
                case "nextTestButton":
                    buttonMash.initializingTypingTest();
                    break;
...
modes.addEventListener("click", function (e) {
    if (
        (<HTMLInputElement>e.target).className != "buttons" &&
        modes.querySelectorAll(".text-button.active").item(0) != <HTMLInputElement>e.target
    ) {
...
quotes.addEventListener("click", function (e) {
    if (
        (<HTMLInputElement>e.target).className != "buttons" &&
        quotes.querySelectorAll(".text-button.active").item(0) != <HTMLInputElement>e.target
    ) {
...
document
    ...
    ?.addEventListener("click", function (e) {
        ...
        console.log(
            ...
            " \nCaret bounds:",
            document.getElementById("caret")?.getBoundingClientRect()
        );
    });
